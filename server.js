...
// ===============================
// 🗓️ GET /get-visits — получение записей на прием
// ===============================
  ...
  try {
    const [rows] = await conn.execute(
      ...
      SELECT 
        ...
        vt.vte_type,
        CONCAT(e.ele_sername, ' ', e.ele_name, ' ', IFNULL(e.ele_patronymic, '')) as doctor_name,
        GROUP_CONCAT(ds.dse_name SEPARATOR ', ') as services
      FROM Visits v
      ...
      JOIN Employees e ON v.ele_id_FK = e.ele_id
      LEFT JOIN Visit_Dental_Services vds ON v.vst_id = vds.vst_id_FK
    ...
    console.error("Ошибка в /get-visits:", err);
    res.status(500).json({ error: "Server error", detail: err.message });
  }
});
